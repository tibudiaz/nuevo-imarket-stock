    ...
    const unsubscribeProducts = onValue(productsRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (lowStockAccessories.length > 0) {
          toast.warning(
            `${lowStockAccessories.length} productos con bajo stock!`,
            {
              description:
                "Revisa el inventario de accesorios para reponer stock.",
            },
          );
        }
        ...
        setDashboardData((prev) => ({
          ...prev,
          totalProducts: productsData.reduce(
            (sum, p) => sum + (p.stock || 0),
            0,
          ),
    ...
    const unsubscribeSales = onValue(salesRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          monthlySales[month] =
            (monthlySales[month] || 0) + Number(sale.totalAmount || 0);
        });
          ...
          sale.items?.forEach((item) => {
            ...
            if (productInfo) {
              totalCostOfGoodsSold +=
                (Number(productInfo.cost) || 0) * item.quantity;
            }
          });
        });
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  +{dashboardData.investmentGrowth.toFixed(1)}% desde el mes
                  pasado
                </p>
              </CardContent>
            </Card>
